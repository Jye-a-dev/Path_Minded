...
export class CurriculumImportsService {
  ...
  async create(payload: Record<string, unknown>): Promise<CurriculumImportResponse> {
    ...
    try {
      const result = await this.pool.query<CurriculumImportEntity>(
        `INSERT INTO curriculum_imports (${cols}) VALUES (${params}) RETURNING *`,
        values,
      );
  ...
  async findAll(query: Record<string, unknown>): Promise<CurriculumImportResponse[]> {
    const { where, values, idx } = this.buildFilter(query);
    const limit = Number(query.limit ?? 20);
    const offset = Number(query.offset ?? 0);

    const result = await this.pool.query<CurriculumImportEntity>(
      `SELECT * FROM curriculum_imports ${where} ORDER BY id DESC LIMIT $${idx} OFFSET $${idx + 1}`,
      [...values, limit, offset],
    );
  ...
  async pagination(query: Record<string, unknown>): Promise<CurriculumImportsPaginationResponse> {
    const { where, values, idx } = this.buildFilter(query);
  ...
  async update(id: string, payload: Record<string, unknown>): Promise<CurriculumImportResponse> {
    const keys = Object.keys(payload);
    if (keys.length === 0) {
  ...
  async remove(id: string): Promise<{ message: string }> {
    const result = await this.pool.query(`DELETE FROM curriculum_imports WHERE id = $1`, [id]);
