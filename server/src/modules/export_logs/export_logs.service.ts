...
export class ExportLogsService {
  ...
  async create(payload: Record<string, unknown>): Promise<ExportLogResponse> {
    ...
    try {
      const result = await this.pool.query<ExportLogEntity>(
        `INSERT INTO export_logs (${cols}) VALUES (${params}) RETURNING *`,
        values,
      );
  ...
  async findAll(query: Record<string, unknown>): Promise<ExportLogResponse[]> {
  ...
  async findOne(id: string): Promise<ExportLogResponse> {
    const result = await this.pool.query<ExportLogEntity>(
      `SELECT * FROM export_logs WHERE id = $1`,
      [id],
    );
  ...
  async update(
    ...
    try {
      const result = await this.pool.query<ExportLogEntity>(
        `UPDATE export_logs SET ${sets} WHERE id = $${keys.length + 1} RETURNING *`,
        [...values, id],
      );
