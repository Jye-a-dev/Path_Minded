...
export class ClassImportRowsService {
  ...
  async create(payload: Record<string, unknown>): Promise<ClassImportRowResponse> {
  ...
  async findAll(query: Record<string, unknown>): Promise<ClassImportRowResponse[]> {
  ...
  async pagination(query: Record<string, unknown>): Promise<ClassImportRowsPaginationResponse> {
    const { where, values, idx } = this.buildFilter(query);
  ...
  async update(id: string, payload: Record<string, unknown>): Promise<ClassImportRowResponse> {
    const keys = Object.keys(payload);
    if (keys.length === 0) {
  ...
  async remove(id: string): Promise<{ message: string }> {
    const result = await this.pool.query(`DELETE FROM class_import_rows WHERE id = $1`, [id]);
