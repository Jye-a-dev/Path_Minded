...
export class CoursePrerequisitesService {
  ...
  async create(payload: Record<string, unknown>): Promise<CoursePrerequisiteResponse> {
  ...
  async findAll(query: Record<string, unknown>): Promise<CoursePrerequisiteResponse[]> {
  ...
  async pagination(query: Record<string, unknown>): Promise<CoursePrerequisitesPaginationResponse> {
    const { where, values, idx } = this.buildFilter(query);
  ...
  async update(id: string, payload: Record<string, unknown>): Promise<CoursePrerequisiteResponse> {
    const keys = Object.keys(payload);
    if (keys.length === 0) {
  ...
  async remove(id: string): Promise<{ message: string }> {
    const result = await this.pool.query(`DELETE FROM course_prerequisites WHERE id = $1`, [id]);
