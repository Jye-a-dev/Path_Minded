...
export class ClassImportsService {
  ...
  async create(payload: Record<string, unknown>): Promise<ClassImportResponse> {
    ...
    try {
      const result = await this.pool.query<ClassImportEntity>(
        `INSERT INTO class_imports (${cols}) VALUES (${params}) RETURNING *`,
        values,
      );
  ...
  async findAll(query: Record<string, unknown>): Promise<ClassImportResponse[]> {
    const { where, values, idx } = this.buildFilter(query);
    const limit = Number(query.limit ?? 20);
    const offset = Number(query.offset ?? 0);

    const result = await this.pool.query<ClassImportEntity>(
      `SELECT * FROM class_imports ${where} ORDER BY id DESC LIMIT $${idx} OFFSET $${idx + 1}`,
      [...values, limit, offset],
    );
  ...
  async pagination(query: Record<string, unknown>): Promise<ClassImportsPaginationResponse> {
    const { where, values, idx } = this.buildFilter(query);
  ...
  async update(id: string, payload: Record<string, unknown>): Promise<ClassImportResponse> {
    const keys = Object.keys(payload);
    if (keys.length === 0) {
  ...
  async remove(id: string): Promise<{ message: string }> {
    const result = await this.pool.query(`DELETE FROM class_imports WHERE id = $1`, [id]);
