...
export function StudentsTab({ selectedClassId, classesForMajor, allPrograms }: StudentsTabProps) {
  const {
    data,
    total,
    page,
  ...
  const handleOpenCreate = () => { setEditingStudent(null); setModalOpen(true); };
  const handleOpenEdit = (item: StudentItem) => { setEditingStudent(item); setModalOpen(true); };
  ...
  const handleDeleteAll = async () => {
    ...
    try {
      ...
      setDeleteAllError(e.response?.data?.message || e.message || "Xóa tất cả sinh viên thất bại.");
    } finally {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (row: StudentItem) => {
        ...
        const statusMap = { ACTIVE: "ĐANG HỌC", GRADUATED: "TỐT NGHIỆP", DROPPED: "THÔI HỌC" };
        return (
          <span className={`inline-flex items-center rounded px-2 py-0.5 text-[10px] font-bold border ${statuses[row.status]}`}>
    ...
    },
    {
      ...
      render: (row: StudentItem) => (
        <span className="text-xs text-slate-400">{getProgramLabel(row.program_id)}</span>
      ),
    },
    {
      header: "Thao tác",
  ...
  return (
    <div className="space-y-4">
      ...
      <Modal
        ...
        title={editingStudent ? "Chỉnh sửa cấu hình Sinh viên" : "Đăng ký hồ sơ Sinh viên"}
        size="lg"
      >
        <StudentForm
        ...
        <div className="space-y-4">
          <div className="rounded-lg bg-rose-500/10 border border-rose-500/20 p-4">
            <p className="text-sm font-semibold text-rose-300">
              ⚠️ Hành động này{" "}
              <span className="font-black underline">không thể hoàn tác</span>.
            </p>
            <p className="mt-1.5 text-xs text-rose-400/80">
              Toàn bộ{" "}
              <span className="font-bold text-rose-300">{total.toLocaleString()} sinh viên</span>{" "}
              trong cơ sở dữ liệu sẽ bị xóa vĩnh viễn, bao gồm tất cả dữ liệu liên kết.
            </p>
          </div>
