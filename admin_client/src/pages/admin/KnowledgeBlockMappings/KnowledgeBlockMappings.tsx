...
import { CreateKnowledgeBlockModal } from "./partials/CreateKnowledgeBlockModal";
import { EditKnowledgeBlockModal } from "./partials/EditKnowledgeBlockModal";

export default function KnowledgeBlockMappings() {
  const { data, statsMap, loading, error, refresh, createItem, updateItem, deleteItem } = useKnowledgeBlockMappings();
  ...
  return (
    <div className="space-y-6">
      ...
      <ExcelColumnMappingSection
        fieldKey="knowledge_block"
        title="Nhận diện Cột Khối kiến thức trong Excel"
        description="Định nghĩa các tiêu đề cột trong file Excel (không phân biệt hoa thường, khoảng trắng) để tự động nhận diện cột chứa dữ liệu Khối kiến thức."
        inputPlaceholder="Thêm tên cột Excel khác (VD: nhóm môn)..."
      />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {sorted.map((item) => (
            <KnowledgeBlockCard
              key={item.id}
              item={item}
              programStats={statsMap.get(item.knowledge_block) ?? []}
