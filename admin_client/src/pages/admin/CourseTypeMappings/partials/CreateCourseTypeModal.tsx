...
interface CreateCourseTypeModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (payload: { course_type: string; label: string; phrases: string[] }) => Promise<unknown>;
}

export function CreateCourseTypeModal({
  isOpen,
  onClose,
  onSubmit,
}: CreateCourseTypeModalProps) {
  ...
  const handleCreate = async () => {
    ...
    setActionError(null);
    try {
      await onSubmit({ course_type: code, label, phrases });
      onClose();
    } catch (err) {
