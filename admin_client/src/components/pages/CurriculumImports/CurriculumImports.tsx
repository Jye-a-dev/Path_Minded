...
export default function CurriculumImports() {
  ...
  const handleConfirmImport = async (id: string) => {
    ...
    try {
      ...
      alert("Curriculum import confirmed and processed successfully!");
      refresh();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to permanently delete this import session?")) {
      try {
        await deleteItem(id);
      } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white m-0">Curriculum Imports</h1>
          <p className="mt-1 text-xs text-slate-400">
            Ingest study syllabi details, register requisites and map program spreadsheets.
          </p>
        </div>
      </div>
      ...
      <DataTable<ImportItem>
        ...
        searchPlaceholder="Search spreadsheet session..."
        rightActions={
          <button
      ...
      <Modal
        ...
        title="Start New Curriculum Import Session"
        size="lg"
      >
