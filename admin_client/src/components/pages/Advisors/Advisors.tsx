...
export default function Advisors() {
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm("Are you sure you want to permanently delete this Advisor?")) {
      try {
        await deleteItem(id);
      } catch (err) {
        alert(err instanceof Error ? err.message : "Failed to delete advisor");
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-white m-0">Advisors Listing</h1>
          <p className="mt-1 text-xs text-slate-400">
            Manage academic advisors, their department groups and link them to system login accounts.
          </p>
        </div>
      </div>
      ...
      <DataTable<AdvisorItem>
        ...
        searchPlaceholder="Search advisor name..."
        rightActions={
          <button
      ...
      <Modal
        ...
        title={editingItem ? "Edit Advisor Info" : "Create Advisor Record"}
      >
        <AdvisorForm
