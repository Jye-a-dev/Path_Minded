          ...
          <header className="relative w-full max-w-5xl mx-auto rounded-2xl border border-slate-800 bg-slate-900/60 shadow-lg shadow-indigo-950/5 px-6 py-3.5 backdrop-blur-md flex items-center justify-between transition-all hover:border-slate-700/80">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="hidden items-center gap-2 rounded-full bg-slate-950/60 px-3 py-1 border border-slate-800 text-[10px] uppercase font-bold tracking-wider text-slate-400 sm:flex select-none">
                ...
                <span>Portal Control Active</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="font-bold text-slate-400 flex items-center gap-1.5">
                  ...
                  PathMinded Academic Control Matrix
                </p>
                <p className="mt-1 text-slate-500 font-medium">Authorized Administrator & Faculty Matrix Advising System.</p>
              </div>
              <div className="flex flex-wrap gap-x-6 gap-y-2 text-slate-400 font-semibold">
                <Link to="/admin" className="hover:text-indigo-400 transition-colors">Dashboard</Link>
                <Link to="/admin/students" className="hover:text-indigo-400 transition-colors">Students</Link>
                <Link to="/admin/curriculum_imports" className="hover:text-indigo-400 transition-colors">Ingestion</Link>
                <a href="https://github.com" target="_blank" rel="noreferrer" className="hover:text-indigo-400 transition-colors">Documentation</a>
              </div>
            </div>
